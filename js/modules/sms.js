...
async function apiCall(endpoint, method = "GET", body = null) {
    ...
    try {
        ...
        try {
            ...
            // Jika server error dan membalas teks mentah, cegah UI dari Crash
...
async function updateSmsBal() {
    ...
    if(isSuccess && json.data) document.getElementById('sms-balance').innerText = getActiveLogic().formatPrice(json.data.balance);
...
// ==========================================
// 5. PEMESANAN & RENDER DOM (Sesuai Struktur Asli)
// ==========================================
export async function buySms(pid, price, name) {
    if (activeProviderKey === "herosms") {
        ...
        // HANYA 3 PROVIDER SESUAI PERMINTAAN (Indosat, Telkomsel, Axis)
        const ops = ["indosat", "telkomsel", "axis"];
        let html = `<div style="padding:15px 10px; font-weight:bold; text-align:center; color:var(--fb-blue); border-bottom:1px dashed var(--fb-border); margin-bottom:10px;">Pilih Provider untuk Harga ${getActiveLogic().formatPrice(price)}</div>`;
        
        ops.forEach(op => {
            html += `<div class="price-item" onclick="executeBuySms('${pid}', ${price}, '${name}', '${op}')">
                <div style="flex: 1; font-weight:bold; padding-left:5px; color:var(--fb-text);">${op.toUpperCase()}</div>
...
export async function executeBuySms(pid, price, name, operator) {
    const pText = getActiveLogic().formatPrice(price);
    ...
    try {
        ...
        if(isSuccess && j.data) { 
            ...
            activeOrders.unshift({
                id: o.id, productId: pid, phone: o.phone_number || o.phone || 'Mencari Nomor...', price: price, otp: null, status: "ACTIVE",
            ...
            showModal("Gagal", j.error?.message || j.message || "Gagal memesan stok.", "alert"); 
            refreshSms(); 
        }
    } catch(e){ refreshSms(); }
}
    ...
    try { 
        ...
        if(isSuccess && j.data) { 
            ...
            activeOrders = serverOrders.map(o => {
                const existing = activeOrders.find(x => x.id == o.id);
                const isHidden = existing ? existing.isHidden : false;
                
                let pid = localStorage.getItem(`xurel_pid_${activeProviderKey}_${o.id}`) || o.product_id;
                let price = o.price || o.cost || o.amount;
                if(!price && pid) { const pMatch = availableProducts.find(p => String(p.id) === String(pid)); if (pMatch) price = pMatch.price; }
                
                const phone = o.phone_number || o.phone || o.phoneNumber || (existing ? existing.phone : 'Mencari Nomor...');

                // Timer Mutlak Server
                let exp = 0;
                if (o.expires_at) {
                    exp = new Date(o.expires_at).getTime();
                    if(isNaN(exp)) exp = parseInt(o.expires_at);
                } else if (o.created_at) {
                    let created = new Date(o.created_at).getTime();
                    if(isNaN(created)) created = parseInt(o.created_at);
                    exp = created + (20 * 60000);
                }

                if (!exp || isNaN(exp)) {
                    exp = parseInt(localStorage.getItem(`timer_${activeProviderKey}_${o.id}`));
                    if (!exp) exp = Date.now() + (20 * 60000);
                }
                localStorage.setItem(`timer_${activeProviderKey}_${o.id}`, exp);

                let lock = o.created_at ? (new Date(o.created_at).getTime() + 120000) : (exp - (18 * 60000));

                return {
                    id: o.id, productId: pid, phone: phone, price: price, 
                    otp: o.otp_code || o.smsCode, status: (o.otp_code || o.smsCode) ? "OTP_RECEIVED" : "ACTIVE",
                    expiresAt: exp, cancelUnlockTime: lock, isAutoCanceling: false, isHidden: isHidden
                };
            });
...
// Struktur HTML Dikembalikan Persis Seperti Orisinal Anda
function renderSmsOrders() {
    ...
    visibleOrders.forEach((o, index) => {
        ...
        let passed2Mins = (o.cancelUnlockTime - now) <= 0;

        let resendState = isSuccess ? '' : 'disabled'; 
        let cancelReplaceState = passed2Mins ? '' : 'disabled';
        ...
        activeOrders.forEach((o, i) => {
            ...
            if(o.cancelUnlockTime - now <= 0 && !o.isHidden) {
                ...
                if(card) {
                    ...
                    if(bC && bC.disabled && !o.otp) bC.disabled = false;
                    if(bR && bR.disabled && !o.otp) bR.disabled = false;
                }
            }
        });
...
export async function replaceSms(id, pid) {
    ...
    try {
        const j = await apiCall('/order-action', 'POST', { id, action: 'cancel' });
        const errStr = JSON.stringify(j).toUpperCase();
        
        // Deteksi pintar Error Palsu dari Server
        let isSuccess = j.success === true || j.status === "success";
        if (!isSuccess && (errStr.includes('NOT_FOUND') || errStr.includes('NO_ACTIVATION') || errStr.includes('NOT FOUND') || errStr.includes('ALREADY'))) {
            isSuccess = true;
        }
        ...
        if (isSuccess) {
            ...
            // Langsung minta any agar paling cepat diganti
            ...
            if (nSuccess && n.data) {
                ...
                localStorage.setItem(`xurel_pid_${activeProviderKey}_${od.id}`, pid); 
                
                activeOrders.unshift({
                    id: od.id, productId: pid, phone: od.phone_number || od.phone || 'Mencari Nomor...', price: orderPrice || 0,
                ...
                showModal("Gagal Pesan Baru", n.error?.message || n.message || "Gagal mengganti stok.", "alert"); 
                renderSmsOrders(); updateSmsBal(); 
            }
        } else { 
            showModal("Gagal Batal", j.error?.message || j.message || "Ditolak server.", "alert"); 
            if (btn) { btn.disabled = false; btn.innerText = "⇄ REPLACE"; } 
        }
    } catch (e) { 
        showModal("Error", "Gagal terhubung.", "alert"); 
        if (btn) { btn.disabled = false; btn.innerText = "⇄ REPLACE"; } 
    }
}
...
export async function actSms(action, id) {
    ...
    if(action === 'cancel') { title = "Batalkan"; msg = "Yakin batalkan pesanan ini?"; type = "danger"; }
    ...
    try {
        ...
        let isSuccess = j.success === true || j.status === "success";
        if (!isSuccess && action !== 'resend' && (errStr.includes('NOT_FOUND') || errStr.includes('NO_ACTIVATION') || errStr.includes('NOT FOUND') || errStr.includes('ALREADY'))) {
            isSuccess = true;
        }
        ...
        if(isSuccess) {
            ...
            if(action === 'resend') showModal("Info", "Permintaan terkirim.", "alert");
        } else { 
            showModal("Gagal", j.error?.message || j.message || "Ditolak server.", "alert"); 
        }
        renderSmsOrders(); updateSmsBal();
    } catch(e) { 
        showModal("Error", "Gagal menghubungi server.", "alert"); 
    }
}
